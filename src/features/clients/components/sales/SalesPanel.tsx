...
type AuditPrintKind = 'invoice' | 'act'

function SaleAuditDetail({
  error,
  isLoading,
  statistic,
}: {
  ...
  const [printDocument, setPrintDocument] = useValueState<ClientPrintDocument | null>(null)
  const [isPrinting, setPrinting] = useValueState(false)
  const [printError, setPrintError] = useValueState<string | null>(null)

  function resolveHistoryNetId(orderItem: SaleOrderItem): string | null {
  ...
  function printDocumentFor(kind: AuditPrintKind, orderItem: SaleOrderItem) {
            ...
            <>
              {printDocument.DocumentURL && (
                <Anchor
                  ...
                  href={printDocument.DocumentURL}
                  rel="noreferrer"
                  target="_blank"
                >
              ...
              {printDocument.PdfDocumentURL && (
                <Anchor
                  className="document-link"
                  href={printDocument.PdfDocumentURL}
                  rel="noreferrer"
                  target="_blank"
                >
...
function LifeCycleRow({ line }: { line: SaleLifeCycleLineItem }) {
  const { t } = useI18n()
        ...
        {canPrint && (
          <Group gap={4} wrap="nowrap">
            <Tooltip label={t('Видаткова накладна')} position="top">
              <ActionIcon
                aria-label={t('Видаткова накладна')}
                color="gray"
                variant="subtle"
...
function ShiftStatusRow({ status }: { status: OrderItemBaseShiftStatus }) {
  const { t } = useI18n()
  const target =
    status.ShiftStatus === OrderItemShiftStatusType.Store
      ? t('На склад')
      : status.ShiftStatus === OrderItemShiftStatusType.Bill
        ? t('В рахунок')
        : ''
