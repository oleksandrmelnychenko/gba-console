...
import { marginDataset, marginRequest } from '../data/marginComparison.test-fixtures'
import { MARGIN_COMPARISON_CAPTIONS, defaultMarginComparison } from '../data/marginComparison'
...
async function choose() {
  ...
  fireEvent.click(await screen.findByRole('option', { name: marginDataset.Name }))
}
...
function previous(from = '2026-06-01', to = '2026-06-30') {
  fireEvent.change(screen.getByLabelText('Маржа: порівняння від'), { target: { value: from } })
  fireEvent.change(screen.getByLabelText('Маржа: порівняння до'), { target: { value: to } })
}
describe('source20 builder and precise contract templates', () => {
  beforeEach(() => {
    vi.clearAllMocks(); localStorage.clear()
    Object.defineProperty(HTMLElement.prototype, 'scrollIntoView', { configurable: true, value: vi.fn() })
    vi.mocked(getReportDatasets).mockResolvedValue([grossDataset, marginDataset]); vi.mocked(getServerReportTemplates).mockResolvedValue([])
  ...
  it('requires two explicit windows and fixed client/CA rows; places settings in body with no unsupported panels', async () => {
    ...
    previous(); expect(screen.getAllByText('Відсотки', { exact: true })).toHaveLength(3)
    expect(screen.getAllByText('Відсоткові пункти', { exact: true })).toHaveLength(1)
    expect(screen.getByLabelText('Параметри порівняння маржі').closest('.reports-stocks-body')).toBeTruthy()
    expect(screen.getByLabelText('Параметри порівняння маржі').closest('.reports-stocks-filter-scroll')).toBeNull()
    ...
    fireEvent.click(screen.getByRole('checkbox', { name: MARGIN_COMPARISON_CAPTIONS[1] })); fireEvent.submit(container.querySelector('form')!)
    await waitFor(() => expect(createStockReport).toHaveBeenCalledOnce())
    const request = vi.mocked(createStockReport).mock.calls[0][0]
    expect(request.marginComparison).toEqual({ ...defaultMarginComparison(), From: '2026-06-01', To: '2026-06-30' })
    expect(request.sorted.Row.map(v => v.type)).toEqual([12, 15]); expect(request.sorted.Col).toEqual([])
    expect(request.sorted.Measurements.map(v => v.Type)).toEqual([55, 57, 58]); expect(request).not.toHaveProperty('xyz'); expect(request).not.toHaveProperty('comparison')
  })
  it('preserves overlapping, unequal and reverse chronological windows without aligning them', async () => {
    ...
    expect(vi.mocked(createStockReport).mock.calls[0][0]).toMatchObject({ from: '2026-03-28', to: '2026-03-30', marginComparison: { From: '2026-10-24', To: '2026-10-26' } })
    ...
    expect(vi.mocked(createStockReport).mock.calls[1][0]).toMatchObject({ from: '2026-06-15', to: '2026-07-15', marginComparison: { From: '2026-07-01', To: '2026-07-31' } })
  })
  it.each([{ field: 9, label: 'Договір' }])('uses exact large lookup identity for $label while descriptive Value remains Int32 zero', async ({ field, label }) => {
    ...
    await waitFor(() => expect(searchDatasetReportValues).toHaveBeenCalledWith(20, field, { limit: 30, offset: 0, value: entity.Id }, expect.any(AbortSignal)))
  ...
  it('reloads caller measure order, exact legacy JSON-string Id and revision; refuses unsupported template before changing form', async () => {
    ...
    const template = { Id: crypto.randomUUID(), Revision: 3, Name: 'Маржа точні періоди', Data: data }
    const invalid = { ...template, Id: crypto.randomUUID(), Name: 'Маржа непідтримана', Data: { ...data, ordering: { Version: 1 } } }
    vi.mocked(getServerReportTemplates).mockResolvedValue([template, invalid]); const { container } = await ready()
    fireEvent.click(screen.getByRole('button', { name: 'Шаблони' })); fireEvent.click(await screen.findByRole('button', { name: /Маржа точні періоди/ }))
    ...
    fireEvent.click(screen.getByRole('button', { name: /Маржа непідтримана/ })); expect(screen.getByText(/Перевірте порівняння маржі/)).toBeTruthy()
    expect((screen.getByLabelText('Маржа: порівняння від') as HTMLInputElement).value).toBe('2026-06-01')
  })
  it('clears comparison state on reset and explicit dataset changes while retaining fixed source20 axes', async () => {
    const { container } = await ready(); await choose(); dates(); previous()
    fireEvent.click(screen.getByRole('button', { name: 'Скинути' }))
    expect((screen.getByLabelText('Маржа: порівняння від') as HTMLInputElement).value).toBe('')
    expect((screen.getByLabelText('Маржа: порівняння до') as HTMLInputElement).value).toBe('')
    expect((screen.getByRole('button', { name: 'Сформувати' }) as HTMLButtonElement).disabled).toBe(true)
    dates(); previous()
    fireEvent.click(screen.getByRole('combobox', { name: 'Набір даних звіту' }))
    fireEvent.click(await screen.findByRole('option', { name: grossDataset.Name }))
    expect(screen.queryByLabelText('Параметри порівняння маржі')).toBeNull()
    fireEvent.submit(container.querySelector('form')!)
    await waitFor(() => expect(createStockReport).toHaveBeenCalledOnce())
    expect(vi.mocked(createStockReport).mock.calls[0][0]).not.toHaveProperty('marginComparison')
    await choose()
    expect((screen.getByLabelText('Маржа: порівняння від') as HTMLInputElement).value).toBe('')
