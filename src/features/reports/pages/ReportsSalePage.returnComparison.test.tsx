...
import { returnRows } from '../data/returnComparison.test-fixtures'
import { RETURN_COMPARISON_EMPTY_STATE } from '../data/returnComparisonSpreadsheet'
...
it('renders authoritative 2dp values from binary XLSX and removes stale totals from filtered CSV', async () => {
  const { container } = viewer(); await upload(returnRows()); await screen.findByText(RETURN_COMPARISON_TITLE)
  expect(screen.getByText('Рядків: 2')).toBeTruthy(); expect(container.querySelector('.reports-sale-table')!.textContent).toContain('-150,00')
  expect(container.querySelectorAll('.reports-client-activity-header')).toHaveLength(1)
  fireEvent.click(screen.getByLabelText('Експорт CSV')); expect(lastCsvSheet().rows.at(-1)?.cells).toEqual(['Загальний підсумок', '', -150, -100, -50, 50])
  ...
  expect(filtered.rows).toEqual([{ kind: 'data', cells: ['Клієнт [1]', 'Договір [201]', -120, -100, -20, 20] }])
  expect(getAdditiveColumns(filtered).every(value => !value)).toBe(true); expect(buildSpreadsheetChartData(filtered, filtered.rows, 5).points.map(p => p.value)).toEqual([20])
})
it('keeps empty selected0/100 grand through search/export and clears stale file after a rejected header', async () => {
  const { container } = viewer(); await upload(returnRows('empty', [0, 3])); await screen.findByText(RETURN_COMPARISON_TITLE)
  expect(screen.getByText(RETURN_COMPARISON_EMPTY_STATE)).toBeTruthy(); expect(screen.getByText('Рядків: 0')).toBeTruthy()
  ...
  const bad = returnRows(); bad.splice(2, 1); await upload(bad, 'bad.xlsx'); await screen.findByText(/Некоректний файл порівняння повернень/)
  expect(screen.queryByText(RETURN_COMPARISON_TITLE)).toBeNull(); expect((screen.getByLabelText('Експорт CSV') as HTMLButtonElement).disabled).toBe(true)
})
