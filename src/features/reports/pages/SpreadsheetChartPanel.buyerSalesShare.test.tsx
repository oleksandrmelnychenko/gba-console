...
it.each([0, 1, 2, 3, 4, 5, 6, 7].flatMap(measure => ['Стовпчики', 'Смуги', 'Лінія'].map(kind => ({ measure, kind }))))('plots selected leaf$measure as $kind, with 2dp and percent value units', ({ measure, kind }) => {
  const raw = buyerShareRows('known', [measure]); const row = raw.find(r => r[1] === 'Договір [201]')!; row[2] = -50
  ...
  expect(points.map((p: SpreadsheetChartPoint) => p.value)).toEqual([-50, [0, 100, -100, -100, 100, 0, 100, 100][measure]])
  expect(points.map((p: SpreadsheetChartPoint) => p.label)).toEqual(['Клієнт [1] · Договір [201]', 'Клієнт [2] · Договір [202]'])
...
it('keeps unknown amount as a line gap and excludes sticky totals', () => {
  const sheet = buildSpreadsheetSheet('Revenue', buyerShareRows('current-unknown', [0]))
