...
export function ReportTopGroupsPanel({ data, dataset, disabled, onChange }: Props) {
  ...
  return <Stack component="section" aria-label="TOP цілих груп" gap="xs" p="sm">
    <Text fw={600}>TOP цілих груп</Text>
    ...
    {!cap ? <Text size="sm" c="dimmed">Сервер не надав можливості TOP груп. Звіти без цього обмеження працюють як раніше.</Text> : null}
    {raw != null ? <Button type="button" variant="subtle" color="gray" size="compact-sm" disabled={disabled}
      onClick={() => onChange(undefined)}>Вимкнути TOP: залишити всі групи</Button> : cap ? <>
      <Button type="button" variant="light" size="compact-sm" disabled={disabled || !initial} onClick={() => { if (initial) onChange(initial) }}>Увімкнути TOP цілих груп</Button>
      {!initial ? <Text size="xs" c="dimmed">Спочатку виберіть поле рядків і увімкнений показник, який сервер дозволяє підсумовувати.</Text> : null}
    </> : null}
