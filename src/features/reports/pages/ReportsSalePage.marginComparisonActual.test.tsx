...
import artifacts from '../data/marginComparison.actual-workbooks.json'
...
function displayedValues(sheet: SpreadsheetSheet) {
  return sheet.rows.map(row => row.cells.slice(2).map((cell, offset) => cell == null || cell === '' ? null
    : getSpreadsheetNumberFormatter(sheet, offset + 2, true)?.format(Number(cell))))
}

it.each(artifacts)('imports actual SQL/C# workbook $name, preserving weighted totals, unknowns and CSV/chart values', async artifact => {
  ...
  expect(leaves.map(row => String(row.cells[1]).match(/\[(\d+)\]$/)?.[1])).toEqual(artifact.contractIds)
  for (let offset = 0; offset < 4; offset += 1) {
    const expected = artifact.expected.filter((_, index) => artifact.kinds[index] === 'data')
      .map(values => values[offset] == null ? null : Number(values[offset]))
    expect(buildSpreadsheetChartData(parsed, parsed.rows, offset + 2).points.map(point => point.value)).toEqual(expected)
  }
  ...
  await screen.findByText(MARGIN_COMPARISON_TITLE)
  ...
  expect(Array.from(tableRows).map(row => Array.from(row.querySelectorAll('td.data-table-cell')).slice(2)
