...
describe('XYZ builder exact periods, bounds and templates', () => {
  ...
  it('uses fixed class/product rows, explicit calendar and selected measures with no extra operations', async () => {
    ...
    expect(screen.getByLabelText('Параметри XYZ').closest('.reports-stocks-filter-scroll')).toBeNull()
    expect(screen.getByLabelText('Параметри XYZ').closest('.reports-stocks-body')).toBeTruthy()
