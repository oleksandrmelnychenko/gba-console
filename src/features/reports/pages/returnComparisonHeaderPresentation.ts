import { RETURN_COMPARISON_NOTE_PREFIXES } from '../data/returnComparisonSpreadsheet'
...
const boundaries = ['Поточний період:', 'Період порівняння:', 'Час читання (UTC):', 'Параметри запиту:',
  'Рядки:', 'Колонки:', 'Показники:', 'Фільтри:', 'Сортування:', 'Стан звіту:', 'УВАГА', '• ', '- ', ...RETURN_COMPARISON_NOTE_PREFIXES]
...
/** Only source18 display joins its eight notes. The raw workbook rows remain unchanged for CSV attribution. */
export function returnComparisonHeaderPresentation(header: SpreadsheetReportHeader): Pick<SpreadsheetReportHeader, 'lines' | 'warnings'> {
  ...
  for (let index = 0; index < header.lines.length; index += 1) {
    ...
    if (RETURN_COMPARISON_NOTE_PREFIXES.some(prefix => content(first).startsWith(prefix))) {
