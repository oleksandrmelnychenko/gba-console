...
it.each([0, 1, 2, 3].flatMap(measure => ['Стовпчики', 'Смуги', 'Лінія'].map(kind => ({ measure, kind }))))('plots selected leaf$measure as $kind, with 2dp and percent value units', ({ measure, kind }) => {
  const raw = returnRows('known', [measure]); const row = raw.find(r => r[1] === 'Договір [201]')!; row[2] = -50
  ...
  expect(points.map((p: SpreadsheetChartPoint) => p.value)).toEqual([-50, [-30, 0, -30, 100][measure]])
...
it('keeps unknown amount as a line gap and excludes sticky totals', () => {
  const sheet = buildSpreadsheetSheet('Revenue', returnRows('current-unknown', [0]))
