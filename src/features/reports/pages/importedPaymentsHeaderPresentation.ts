import { IMPORTED_PAYMENTS_NOTE_PREFIXES } from '../data/importedPaymentsSpreadsheet'
import type { SpreadsheetReportHeader } from '../types'

const boundaries = ['Період:', 'Час читання (UTC):', 'Параметри запиту:',
  'Рядки:', 'Колонки:', 'Показники:', 'Фільтри:', 'Сортування:', 'Стан звіту:', 'УВАГА', '• ', '- ', ...IMPORTED_PAYMENTS_NOTE_PREFIXES]
...
/** Only source14 display joins its six notes. The raw workbook rows remain unchanged for CSV attribution. */
export function importedPaymentsHeaderPresentation(header: SpreadsheetReportHeader): Pick<SpreadsheetReportHeader, 'lines' | 'warnings'> {
  ...
  for (let index = 0; index < header.lines.length; index += 1) {
    ...
    if (IMPORTED_PAYMENTS_NOTE_PREFIXES.some(prefix => content(first).startsWith(prefix))) {
