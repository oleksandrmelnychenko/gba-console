import { REVENUE_COMPARISON_NOTE_PREFIXES } from '../data/revenueComparisonSpreadsheet'
...
const boundaries = ['Поточний період:', 'Період порівняння:', 'Час читання (UTC):', 'Параметри запиту:',
  'Рядки:', 'Колонки:', 'Показники:', 'Фільтри:', 'Сортування:', 'Стан звіту:', 'УВАГА', '• ', '- ', ...REVENUE_COMPARISON_NOTE_PREFIXES]
...
/** Only source16 display joins its eight notes. The raw workbook rows remain unchanged for CSV attribution. */
export function revenueComparisonHeaderPresentation(header: SpreadsheetReportHeader): Pick<SpreadsheetReportHeader, 'lines' | 'warnings'> {
  ...
  for (let index = 0; index < header.lines.length; index += 1) {
    ...
    if (REVENUE_COMPARISON_NOTE_PREFIXES.some(prefix => content(first).startsWith(prefix))) {
