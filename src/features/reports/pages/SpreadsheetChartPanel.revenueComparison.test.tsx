...
it.each([0, 1, 2, 3].flatMap(measure => ['Стовпчики', 'Смуги', 'Лінія'].map(kind => ({ measure, kind }))))('plots selected leaf$measure as $kind, with 2dp and percent value units', ({ measure, kind }) => {
  const raw = revenueRows('known', [measure]); const row = raw.find(r => r[1] === 'Договір [201]')!; row[2] = -50
  ...
  expect(points.map((p: SpreadsheetChartPoint) => p.value)).toEqual(measure === 3 ? [-50, 100, 100] : [-50, 0, 0])
  expect(points.map((p: SpreadsheetChartPoint) => p.label)).toEqual(['Клієнт [1] · Договір [201]', 'Клієнт [1] · Договір [202]', 'Невідомий клієнт · Договір [203]'])
...
it('keeps unknown amount as a line gap and excludes sticky totals', () => {
  const sheet = buildSpreadsheetSheet('Revenue', revenueRows('current-unknown', [0]))
