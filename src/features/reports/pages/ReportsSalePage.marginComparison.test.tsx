...
it('renders authoritative 2dp values from binary XLSX and removes stale totals from filtered CSV', async () => {
  const { container } = viewer(); await upload(marginRows()); await screen.findByText(MARGIN_COMPARISON_TITLE)
  expect(screen.queryByLabelText('Від')).toBeNull(); expect(screen.queryByLabelText('До')).toBeNull();
  expect(screen.getByText('Рядків: 2')).toBeTruthy(); expect(container.querySelector('.reports-sale-table')!.textContent).toContain('15,00')
  expect(container.querySelectorAll('.reports-client-activity-header')).toHaveLength(1)
  fireEvent.click(screen.getByLabelText('Експорт CSV')); expect(lastCsvSheet().rows.at(-1)?.cells).toEqual(['Загальний підсумок', '', 15, 10, 5, 50])
  ...
  expect(filtered.rows).toEqual([{ kind: 'data', cells: ['Клієнт [1]', 'Договір [201]', 30, 20, 10, 50] }])
...
it('keeps empty selected0/100 grand through search/export and clears stale file after a rejected header', async () => {
  const { container } = viewer(); await upload(marginRows('empty', [0, 3])); await screen.findByText(MARGIN_COMPARISON_TITLE)
  expect(screen.getByText(MARGIN_COMPARISON_EMPTY_STATE)).toBeTruthy(); expect(screen.getByText('Рядків: 0')).toBeTruthy()
  ...
  const bad = marginRows(); bad.splice(2, 1); await upload(bad, 'bad.xlsx'); await screen.findByText(/Некоректний файл порівняння маржі/)
  expect(screen.queryByText(MARGIN_COMPARISON_TITLE)).toBeNull(); expect((screen.getByLabelText('Експорт CSV') as HTMLButtonElement).disabled).toBe(true)
})
