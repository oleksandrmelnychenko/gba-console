...
type BuilderValues = {
  dataSource: number; from: string; to: string; ordering: unknown; filterExpression: unknown
...
export function buildReportBuilderRequest(values: BuilderValues): ReportRequestBody {
  const { dataSource, from, to, ordering, filterExpression, valuationClientAgreementId, rowGroups, colGroups, measurements, selections } = values
