...
export function datasetMeasurements(dataset: ReportDataset | undefined, selected: ReportMeasurementSelection[] = []): ReportMeasurementGroup[] {
  ...
  const checked = new Set(selected.flatMap(item => ((dataset.DataSource === 13 || dataset.DataSource === 14 || dataset.DataSource === 15) ? item.IsChecked !== false : item.IsChecked) ? [item.Type] : []))
...
export function defaultDatasetRequest(dataset: ReportDataset, from: string, to: string): ReportRequestBody {
  ...
  return { dataSource: dataset.DataSource, ...(dataset.DataSource === 15 ? { xyz: defaultXyzOptions() } : {}), ...(dataset.DataSource === 13 ? { comparison: { Version: 1, From: '', To: '' } } : {}), from: dataset.PeriodSupported === false ? '' : from,
...
export function datasetConfigurationError(data: ReportRequestBody, dataset: ReportDataset | undefined): string | null {
  ...
  const unsupported = [
    ...
    ...data.selections.flatMap(item => dataset.DataSource === 15 && item.IsChecked === false ? [] : (!item.IsChecked || filterTypes.has(item.SelectedField?.Type)) && conditionTypes.has(item.FilterCondition?.Type)
...
export function datasetPresetRequest(dataset: ReportDataset, id: DatasetReportPresetId, current: ReportRequestBody) {
  ...
  const preservedOptions = { ...cloneXyzAliases(current), ...(Object.hasOwn(current, 'comparison') ? { comparison: structuredClone(current.comparison) } : {}),
