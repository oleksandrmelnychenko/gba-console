...
export function buildSpreadsheetSheet(name: string, rows: SpreadsheetCellValue[][], format: 'workbook' | 'flat' = 'workbook'): SpreadsheetSheet {
  ...
  const sheet = validateRateComparisonSheet(validateMarginComparisonSheet(validateReturnComparisonSheet(validateBuyerSalesShareSheet(validateRevenueComparisonSheet(validateSalesXyzSheet(validateImportedPaymentsSheet(validateClientComparisonSheet(validateClientActivitySheet({
    name,
...
export function filterSheetRows(
  ...
  if ((isClientActivitySheet(sheet) && sheet.header?.lines.includes(CLIENT_ACTIVITY_EMPTY_STATE))
    ...
    || (isPaymentComparisonSheet(sheet) && sheet.header?.lines.includes(PAYMENT_COMPARISON_EMPTY_STATE))
  ...
  const from = supportsSpreadsheetDateFilters(sheet) ? dateFrom : ''
  const to = supportsSpreadsheetDateFilters(sheet) ? dateTo : ''
