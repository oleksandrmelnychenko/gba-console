...
| 51 | P1 clients | clients | `/clients/edit/:netid/:step` | `EditClientView` | ui | all tabs migrated: general/contact/bank, pricing, structure, perfect-client, sales, recommendations, e-commerce; editable type/role panel; pricing discount tree preserves nested product subgroups and guards dirty agreement switches with apply/cancel |
...
| 100 | P2 products | products | `/products` | `ProductsView` | ui | list/search/filters/table and readonly detail route `/products/:netId` wired; price rows show retail/base/discount breakdown; product upload blocks duplicate pricing mappings and sends one server-compatible file |
...
| 105 | P5 sales-reports | sales | `/sales/ukraine/all` | `SalesManagerDashboard` | ui | SalesAndInvoices tab fully migrated (feature `sales-ukraine`): grid + filters (date/status/manager/org/client/search) + paginated `/sales/all/filtered`; faithful row actions (documents incl. A/B/C history, discount, ship, will-not-ship, unlock, transporter panel, TTN); SaleMasterView editor (open/edit qty/add/remove/switch agreement/delivery/convert/merges) + reassign + copy-data + new-sale via current cart. Pricing/discount parity: one-time discount lifecycle normalized to New/Packaging, base discount displayed separately, new-sale product selection loads agreement current price + calculated discount pricings + product-specific reservation, and cart/realtime reservation state is guarded against stale reloads. ALL other tabs now migrated as standalone routes: Debtors (/sales/ukraine/debtors), Interest=preorders (/interest), Cart-reserve (/cart-reserve), Client-product-movement (/client-product-movement), Offers (/offers), Prediction (/prediction), Charts (/sales/charts); Returns = /sales/ukraine/all/returns/new. All adversarially parity-verified. Optional remaining: unified tab-shell UX (routes currently standalone). |
