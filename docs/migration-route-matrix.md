...
| 72 | P4 supply-warehouse | orders | `/orders/ukraine/all/edit/:id` | `SupplyLogisticsPage` | ui | direct order logistic/detail route wired with status badges, supplier/org/agreement/totals, delivery type save, permission-gated approve and amount edit, delivery document upload/status/file removal, and linked invoice/spec/product-income actions |
...
| 96 | P2 products | product-storages | `/products/storages` | `ProductStoragesView` | ui | read/list/export slice wired to storage availability endpoints with permission-gated bulk action preview, editable quantities, validation, transfer, write-off, and return flows |
