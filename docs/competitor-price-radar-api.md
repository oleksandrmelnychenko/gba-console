...
```json
{
  ...
  "sources_scanned": ["prom", "hotline", "avtopro"],
...
2. `gba-pricing` calls `claude-sonnet-5` with Anthropic `web_search_20260318`, localized to Ukraine and restricted to selected domains unless `google` (the wider web) is selected.
...
5. Results are deduplicated, sorted by similarity and price, capped at 18 offers, and cached in Redis for 15 minutes by model + normalized query + selected sources.
