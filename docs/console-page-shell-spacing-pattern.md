# Патерн: відступи/бордер врапера консольних сторінок
...
3. **Окрема кнопка «Оновити» в фільтр-барі поряд з `Paginator`, який теж має
   свою іконку оновлення** (дублікат) → потрібен **фікс розміщення `Paginator`**
   (п.3).
4. **Під карткою/таблицею — великий «мертвий» білий проміжок** (картка чи
   таблиця не дотягується до низу фрейма, хоч місця вистачає) → потрібен
   **фікс розтягування таблиці по висоті** (п.4).
...
## 3. Пагінатор у фільтр-барі (не окремим рядком під таблицею)
...
## 4. Таблиця розтягнута по висоті (замість `maxHeight` з фіксованим числом)
...
- [ ] `tsc -b --noEmit` чистий
- [ ] Візуально: скріншот у браузері, немає «голої» картки
- [ ] Перевірено хоча б з одним реальним рядком даних, не тільки на порожній таблиці
- [ ] Якщо сторінка має пагінацію, `Paginator` живе в `app-filter-actions`,
      окремої кнопки «Оновити» й окремого рядка пагінації під таблицею немає
      (розділ 3)
...
- `6f20474` Fix product income documents page shell spacing and border
- `90afc5e` Fix product capitalizations page shell spacing and border
- `src/features/product-transfers/pages/ProductTransfersPage.tsx` — повний
  приклад розділів 1, 2, 3 і 4 разом (fudge-less сторінка без жодного
